class GlobalProjectsController < ApplicationController
  ...
  def index
    ...
    if q.present?
      ...
      search_by_projects = Project.published.ransack(name_or_title_or_description_eq_any: query.split).result
      ...
      search_by_tags = Project.published.ransack(tags_name_eq_any: query.split).result

      search_by_states = Project.published.ransack(recipe_states_description_eq_any: query.split).result
