...
if @project.owner_type == 'User'
  ...
  if @prev.nil?
    ...
    json.prev_url (root_url.chop + api_v0_user_project_recipe_state_annotation_path(@owner, @project, @state, @prev))
  end
  if @next.nil?
    json.next_url nil
  else
    json.next_url (root_url.chop + api_v0_user_project_recipe_state_annotation_path(@owner, @project, @state, @next))
  end
else
  ...
  if @prev.nil?
    ...
    json.prev_url (root_url.chop + api_v0_group_project_recipe_state_annotation_path(@owner, @project, @state, @prev))
  end
  if @next.nil?
    json.next_url nil
  else
    json.next_url (root_url.chop + api_v0_group_project_recipe_state_annotation_path(@owner, @project, @state, @next))
  end
end
